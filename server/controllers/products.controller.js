var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
System.register("server", ["mysql/promise", "express"], function (exports_1, context_1) {
    "use strict";
    var promise_1, app, express_1;
    var __moduleName = context_1 && context_1.id;
    return {
        setters: [
            function (promise_1_1) {
                promise_1 = promise_1_1;
            },
            function (express_1_1) {
                express_1 = express_1_1;
            }
        ],
        execute: function () {
            // const express = require('express');
            app = express_1["default"]();
            exports_1("default", db = promise_1["default"].createConnection({
                host: 'localhost',
                user: 'root',
                password: 'root',
                database: 'furniture_shop'
            }));
        }
    };
});
System.register("controllers/products.controller", ["server"], function (exports_2, context_2) {
    "use strict";
    var server_1, ProductController;
    var __moduleName = context_2 && context_2.id;
    return {
        setters: [
            function (server_1_1) {
                server_1 = server_1_1;
            }
        ],
        execute: function () {
            ProductController = /** @class */ (function () {
                function ProductController() {
                }
                // Assuming you have a `db` object representing your MySQL connection
                ProductController.getAllProducts = function (req, res) {
                    return __awaiter(this, void 0, void 0, function () {
                        var sql;
                        return __generator(this, function (_a) {
                            try {
                                sql = "select * from products";
                                server_1["default"].query(sql, function (err, result) {
                                    if (err) {
                                        console.error(err);
                                        res.status(500).send(err);
                                    }
                                    else {
                                        res.status(200).send(result);
                                    }
                                });
                            }
                            catch (error) {
                                return [2 /*return*/, res.status(500).send(error)];
                            }
                            return [2 /*return*/];
                        });
                    });
                };
                ProductController.getProduct = function (req, res) {
                    return __awaiter(this, void 0, void 0, function () {
                        var id, sql;
                        var _this = this;
                        return __generator(this, function (_a) {
                            try {
                                id = req.params.id;
                                sql = "select * from products where product_id = ?";
                                server_1["default"].query(sql, [id], function (error, results) {
                                    _this.getProductQuery(error, results, res);
                                });
                            }
                            catch (error) {
                                return [2 /*return*/, res.status(500).send(error)];
                            }
                            return [2 /*return*/];
                        });
                    });
                };
                ProductController.createProduct = function (req, res) {
                    return __awaiter(this, void 0, void 0, function () {
                        var id, sql;
                        var _this = this;
                        return __generator(this, function (_a) {
                            try {
                                id = req.params.id;
                                sql = "select * from products where product_id=?";
                                server_1["default"].query(sql, [id], function (error, results) {
                                    _this.getProductQuery(error, results, res);
                                });
                            }
                            catch (error) {
                                return [2 /*return*/, res.stats(500).send(error)];
                            }
                            return [2 /*return*/];
                        });
                    });
                };
                ProductController.updateProduct = function (req, res) {
                    return __awaiter(this, void 0, void 0, function () {
                        var id, _a, name_1, description, price, sql;
                        return __generator(this, function (_b) {
                            try {
                                id = req.params.id;
                                _a = req.body, name_1 = _a.name, description = _a.description, price = _a.price;
                                sql = "update products set name = ?, description = ?, price = ? where product_id = ?";
                            }
                            catch (error) {
                                console.error(error);
                            }
                            return [2 /*return*/];
                        });
                    });
                };
                ProductController.getProductQuery = function (error, results, res) {
                    if (error) {
                        console.error(error);
                        res.status(500).send(error);
                    }
                    else if (results.length === 0) {
                        res.status(404).send("Product not found");
                    }
                    else {
                        res.status(200).send(results[0]);
                    }
                };
                return ProductController;
            }());
            exports_2("ProductController", ProductController);
        }
    };
});
